...
class QueryTests(TestCase):
    ...
    def test_get_portions_by_meal_id(self):
        portions = get_portions_by_meal_id(request=self.get_default_request(), meal_id=1)
    ...
    def test_empty_get_summary_of_consume(self):
        ...
        daily_totals = get_summary_of_consume(request=self.get_default_request(), date=date)
    ...
    def test_get_meals_data(self):
        ...
        meals_data = get_meals_data(request=self.get_default_request(), from_date=from_date, to_date=to_date)
        result = [
